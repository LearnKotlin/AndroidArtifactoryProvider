package com.learnkotlin.canvasglasslibrary
...
class CanvasGlass(context: Context?, attrs: AttributeSet?) : AppCompatImageView(context, attrs) {
    ...
    private var boxSize = resources.getDimensionPixelSize(R.dimen.boxsize)
    ...
    fun setBoxSize(resource: Int){
        boxSize = resources.getDimensionPixelSize(resource)
        invalidate()
    }

    fun setBoxStroke(resource: Int){
        paint.strokeWidth = resources.getDimensionPixelSize(resource).toFloat()
        invalidate()
    }
